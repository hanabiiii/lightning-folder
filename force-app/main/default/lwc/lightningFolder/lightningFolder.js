...
export default class LightningFolder extends LightningElement {
    ...
    renderedCallback() {
        ...
        .then(() => {
            Promise.all([
                ...
                loadStyle(this, FANCYTREE + `/skin-${this.fancyTreeSkin}/ui.fancytree.min.css`)
            ])
            .then(() => {
            ...
            try {
                ...
                const fancyTree = j$(treeEl).fancytree({
                    ...
                    init: (event, data) => {
                        ...
                        this.isLoading = false;
                        if (this.selectedFolder) this.toggleTargetFolder(this.selectedFolder);
                    },
                    icon: (event, data) => {
                        if (data.node.folder && this.icon === 'salesforce') {
                            const folderHref = '/_slds/icons/doctype-sprite/svg/symbols.svg#folder';
                            return {
    ...
    toggleTargetFolder(selectedFolder) {
        ...
        if (fancyTree && selectedFolder) {
            ...
            if (selectedFolder && selectedNodes && selectedNodes.filter(filterSelectedNodes).length > 0) {
            ...
            .then(data => {
                ...
                const findSelectedNode = (part) => {
                    ...
                    if (nodeKey == selectedFolder || 
                        (node.data && node.data.isWorkspace && node.data.workSpaceId == selectedFolder)) {
                        node.setSelected(true);
                        node.setActive(true);
                        // console.log('found node w key', nodeKey);
                        cmp.set('v.isLoadingFolderPath', false);
                    } else {
                        const forceReload = this.folderIdLoadedChild.indexOf(nodeKey) == -1 ? true : false;
                        node.load(forceReload).then(() => {
    ...
    fireSelectEvent(folderId, folderLabel) {
        try {
            ...
            const onSelect = new CustomEvent('select', { detail: {
                folderId: folderId,
                folderLabel: folderLabel,
                parentFolderId: parentFolder.isWorkspace ? parentFolder.workSpaceId : parentFolder.id
            }});
